...
const newUser = ref({
  ...
  email: "",
  password: "",
  // POST /users (the only user-creation endpoint) binds role as
  // `oneof=student teacher`, so admins cannot be created from this form.
...
const handleDeleteUser = async (user: User) => {
  if (
    !confirm(
      `Remover o usuário "${user.name}"? Esta ação também remove seus empréstimos e não pode ser desfeita.`,
    )
  ) {
    return;
  }
...
const filteredUsers = computed(() => {
  return users.value.filter((u) => {
    const matchesSearch =
      u.name.toLowerCase().includes(searchFilter.value.toLowerCase()) ||

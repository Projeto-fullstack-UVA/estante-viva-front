...
const loadData = async () => {
  try {
    ...
    error.value =
      err instanceof Error ? err.message : "Erro ao carregar livros";
  } finally {
...
const lentCount = computed(
  () => books.value.filter((b) => b.status === "lent").length,
);
...
const handleBorrow = async (bookId: number) => {
  ...
  try {
    ...
    error.value =
      err instanceof Error ? err.message : "Erro ao pegar livro emprestado";
  }
};
  ...
  <AuthenticatedLayout>
    <div class="page-head">
      ...
      <p class="page-description">
        Explore o acervo completo, filtre por título, autor ou status e encontre
        seu próximo livro.
      </p>
    </div>
        ...
        <div class="book-card__foot">
          <template v-if="getActiveLoanForBook(book.id)">
            <p class="book-card__note">
              Devolução até
              <strong>{{
                formatDate(getActiveLoanForBook(book.id)!.return_date)
              }}</strong>
            </p>
            <button
              type="button"
